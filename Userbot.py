...
async def add_deal(client, message):
    if not message.reply_to_message:
        return await message.reply_text("❌ Please reply to a DEAL INFO message!")
    ...
    if not amount_match:
        return await message.reply_text("❌ Amount not found in the message!")
...
async def complete_deal(client, message):
    ...
    if not deal_info:
        return await message.reply_text("❌ This deal was never added with /add!")
    if deal_info["completed"]:
        return await message.reply_text("❌ This deal is already completed!")
